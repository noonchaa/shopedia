...
const Dash = ({left,click,user}) => {
    ...
    useEffect(()=>{
        getDoc(doc(db,'admins',!user?'admin':user.email)).then((doc)=>{setAdmin(doc.data())}).catch(()=>{setAdmin(null)})
    ...
    return(
        <nav className={left==false?'hidden':'fixed top-0 bg-gray-200 shadow-xl w-80 z-50 min-h-screen p-4'}>
            ...
            <div className='flex mb-4 border-t border-black pt-4 items-end'>
                <Image src={admin.photo} width={32} height={32} alt='Logo' className='rounded-full'/>
                <h1 className='text-xl ml-2 font-medium'>{admin.name}</h1>
            </div>
            }
