...
const RefundData = ({data}) => {
    const {order_id,item_details,status,ongkir,gross_amount} = data
    ...
    return(
        <div className='bg-white border-2 border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 p-6 relative'>
            ...
            <p className="font-semibold tracking-tight overflow-hidden mb-4 dark:text-white capitalize">Ongkir : Rp. {Number(ongkir.price).toLocaleString('ID',{'currency':'IDR'})}</p>
            <p className="font-semibold tracking-tight overflow-hidden mb-8 dark:text-white capitalize">Total : Rp. {Number(gross_amount).toLocaleString('ID',{'currency':'IDR'})}</p>
            <p className='mb-10 text-red-600'>**Hapus Data apabila pengembalian dana telah diterima pelanggan</p>
